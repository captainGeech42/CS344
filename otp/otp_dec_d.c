...
void process(int socket_fd) {
    ...
    if (recv_data(socket_fd, &ciphertext) < 0) {
        fputs("error getting ciphertext from client\n", stderr);
        exit(1);
    }
#ifdef DEBUG
    else {
        printf("got ciphertext from server: %s\n", plaintext);
    }
#endif
    ...
    if (!send_data(socket_fd, ciphertext, strlen(ciphertext))) {
        fputs("error sending plaintext to client\n", stderr);
        exit(1);
    }
...
// generate plaintext (saved in ciphertext)
void decode(char *ciphertext, const char *key) {
    // loop through the ciphertext and do the decoding
    int i, p, k, c;
    for (i = 0; i < strlen(ciphertext)-1; i++) {
        p = get_int(ciphertext[i]);
        k = get_int(key[i]);
