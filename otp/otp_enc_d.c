...
void process(int socket) {
    ...
    char *plaintext = NULL;
    char *key = NULL;
    int text_len = recv_data(socket, plaintext, key);
...
int recv_data(int socket, char *plaintext, char *key) {
    ...
    while (strpbrk(plaintext, "\n") == NULL) {
        ...
        // read in more data
        bytes_read = read(socket, plaintext+(BUFFER_BASE*(base_count-1)), BUFFER_BASE-1);
        if (bytes_read < 0) {
            fprintf(stderr, "error reading from socket: %s\n", strerror(errno));
            return -1;
        }
    ...
    // get data from the socket
    bytes_read = read(socket, key, BUFFER_BASE-1);
    if (bytes_read < 0) {
        fprintf(stderr, "error reading from socket: %s\n", strerror(errno));
        return -1;
    }
